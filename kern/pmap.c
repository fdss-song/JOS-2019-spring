...
mem_init(void)
{
	...
	boot_map_region(kern_pgdir, KERNBASE, ROUNDUP(0x100000000 - KERNBASE, PTSIZE), 0, PTE_W);
...
boot_map_region_large(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	// `va + size` may not be aligned
	size_t i, pgs = (ROUNDUP(va + size, PTSIZE) - va) / PTSIZE;
